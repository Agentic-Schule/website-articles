...
Die meisten mehrstufigen Agenten sind eine gerade Linie. Schritt eins, Schritt zwei, Schritt drei, und jeder wartet, bis der vorige fertig ist. Das ist bequem und oft auch richtig. Manchmal aber ist es einfach nur langsam, und zwar aus einem Grund, den man leicht übersieht: Die Hälfte der Schritte hätte gar nicht warten müssen.

Ein Graph macht diese Struktur sichtbar. Er hat nur zwei Bausteine:
...
> **💡 Die Faustregel:** Fließen Daten von A nach B, ist das eine echte Kante, die Reihenfolge bleibt. Fließen keine Daten, ist es keine Kante, und das Warten ist verschenkt. Dann laufen A und B nebeneinander.
...
Ausgerechnet LangChain, die Firma hinter dem verbreiteten Agenten-Framework LangGraph, [ordnet „Graph Engineering" selbst ein](https://www.langchain.com/blog/3-years-of-graph-engineering-with-langgraph): es sei „the latest term to come out of X's AI content factory", in einer Reihe mit Prompt Engineering, Context Engineering und Loop Engineering. Ein Buzzword also, aber eines, das laut LangChain echte Entwurfsentscheidungen benennt („they do describe real challenges and design decisions builders face").

Denn dieselben Knoten und Kanten stecken längst in echter Software. Genau die Idee, ein Agentensystem als Graph zu modellieren, hat LangChain vor drei Jahren zu LangGraph geführt, das heute über 65 Millionen Mal im Monat geladen wird. Die Definition dort deckt sich mit unserer: „nodes do work" (Knoten erledigen die Arbeit), „edges define what happens next" (Kanten bestimmen, was als Nächstes kommt), das Ganze als Zustandsmaschine.

Ein Stolperstein bleibt: „Graph" ist in der KI-Welt überladen. Das Wort meint auch etwas ganz anderes, nämlich Wissensgraphen für die Suche, wie in Microsofts [GraphRAG](https://www.microsoft.com/en-us/research/blog/graphrag-unlocking-llm-discovery-on-narrative-private-data/), wo aus Texten Entitäten und Beziehungen extrahiert werden. Damit hat unser Thema nichts zu tun. Beim Graph Engineering geht es um Abläufe zwischen Agenten, bei GraphRAG um die Struktur von Wissen.

Das Prinzip zu kennen ist das eine, es umzusetzen das andere. In Claude Code heißt das Werkzeug dafür Dynamic Workflows, und das Wort „Graph" kommt in der Dokumentation nicht ein einziges Mal vor.

## Das Werkzeug dahinter: Dynamic Workflows

Die Dokumentation beschreibt sie [in einem Satz](https://code.claude.com/docs/en/workflows):
...
**Ausprobieren kostet einen Befehl.** Am schnellsten sieht man es an `/deep-research`, dem mitgelieferten Workflow für Recherchefragen:
...
Einen eigenen Workflow bekommst du, indem du ihn im Prompt verlangst, etwa „baue einen Workflow, der …". Verfügbar ist das auf allen bezahlten Plänen; auf Pro schaltest du die Zeile _Dynamic workflows_ vorher in `/config` ein.
...
Ohne den Lärm bleibt eine erfreulich unspektakuläre Wahrheit übrig. Das Prinzip hinter „Graph Engineering" ist alt und einfach: unabhängige Arbeit parallel laufen lassen. Das Werkzeug dafür ist echt, gut dokumentiert und steckt bereits in deinem Claude Code.
