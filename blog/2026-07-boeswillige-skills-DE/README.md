...
Und, das ist der unangenehmste Teil, die Datei wird benutzt. Wie oft, lässt sich von außen nicht sagen. Wer den Skill global in sein `~/.claude`-Verzeichnis installiert, taucht nirgends auf, diese Fälle sind schlicht nicht zählbar. Sichtbar werden nur die, die ihn in die Versionsverwaltung eingecheckt haben, [so wie in diesem Projekt](https://github.com/RudyCity/superagent/blob/main/.agents/skills/brand-landingpage/SKILL.md). Dieselbe Datei, samt der Adresse, die sich jederzeit wieder scharf stellen lässt.
