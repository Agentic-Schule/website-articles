...
Und die Ursache ist ärgerlich klein, fast schon tragisch. Den Schlüssel erzeugen sollte ein eigens verbauter Hardware-Zufallsgenerator, ein TRNG (True Random Number Generator). Der kam nie zum Zug. Ein Build- und Link-Fehler ließ die Erzeugung stattdessen auf einen Software-Generator aus der Bibliothek MicroPython zurückfallen. Dagegen gab es sogar eine Sicherung: ein [`#error "get a HW TRNG plz"`](https://github.com/coinkite/libngu/blob/52a18db224f7770db25df3c58fc410bfad0d7f32/ngu/random.c#L29), das den Build hätte abbrechen sollen. Nur stand davor [`#ifndef MICROPY_HW_ENABLE_RNG`](https://github.com/coinkite/libngu/blob/52a18db224f7770db25df3c58fc410bfad0d7f32/ngu/random.c#L28), und `#ifndef` prüft nur, **ob** ein Makro definiert ist, nicht welchen Wert es hat. Coinkite hatte es [auf `0` gesetzt](https://github.com/Coldcard/firmware/blob/a76fea2ba68c88bae8d9767c155e6893a239696e/stm32/COLDCARD/mpconfigboard.h#L77), weil die Coldcard ihren eigenen Generator mitbringt. „Auf 0 definiert" zählt aber als „definiert", also schlug die Sicherung nie an. Der eigentliche Fallback steckt eine Ebene tiefer: Mit dem Schalter auf 0 liefert MicroPythons `rng_get()` deterministischen Software-Zufall, gespeist aus Chip-Seriennummer und Timer-Registern. Genau an diese Funktion band sich der Schlüsselcode, statt an Coldcards eigenen Hardware-Generator. Der Hersteller stellt klar: „There was no intentional weak-entropy fallback." Aber das war auch nur noch ein schwacher Trost, und die gesamte Kommunikation wurde mit weiteren irrelevanten Rechtfertigungen über Social Media zur absoluten Katastrophe.
