...
Und die Ursache ist ärgerlich klein, fast schon tragisch. Den Schlüssel erzeugen sollte ein eigens verbauter Hardware-Zufallszahlengenerator, ein TRNG (True Random Number Generator). Der kam nie zum Zug. Das war kein Zufall und kein Build-Ausrutscher: In allen drei Board-Konfigurationen der Coldcard steht [`MICROPY_HW_ENABLE_RNG` fest auf `0`](https://github.com/Coldcard/firmware/blob/bcc2c382a324690a2fcf972c0bac3b79bf923f7b/stm32/COLDCARD/mpconfigboard.h#L77). Genau dieser Wert wählt in MicroPython den Software-Fallback: Steht das Makro auf 0, gibt [`rng_get()`](https://github.com/Coldcard/micropython/blob/4107246f8a080807b62c3b4838e71e812ea68b6f/ports/stm32/rng.c#L64-L98) einen Pseudozufallsgenerator zurück, den die Quelle im Kommentar selbst „not really ideal" nennt, gespeist aus Chip-Seriennummer und ein paar Timer-Registern. An genau dieses `rng_get()` band sich der Schlüsselcode. Den guten Hardware-TRNG gab es durchaus, aber er steckte in Coldcards eigenem RNG-Code, und die Schlüsselerzeugung rief ihn nie auf.

Eine Sicherung hätte das abfangen sollen: ein [`#error "get a HW TRNG plz"`](https://github.com/switck/libngu/blob/537519a829259622ea6b0334fbafd6cae852852f/ngu/random.c#L29), das den Build abbricht, wenn kein Hardware-Zufall da ist. Nur stand davor [`#ifndef MICROPY_HW_ENABLE_RNG`](https://github.com/switck/libngu/blob/537519a829259622ea6b0334fbafd6cae852852f/ngu/random.c#L28). Und weil hier ein Präprozessor-Makro im Spiel ist und keine normale Variable, prüft `#ifndef` bloß, **ob** das Makro definiert ist, nicht welchen Wert es hat. „Auf 0 definiert" zählt als „definiert", also schlug die Sicherung nie an. Doch selbst eine korrigierte Prüfung wäre nur eine Makro-Assertion geblieben, und die sagt nichts darüber, woher der Zufall am Ende wirklich kommt. Nötig gewesen wäre ein Test, der end-to-end nachweist, dass der Schlüssel tatsächlich aus der Hardware stammt. Der Hersteller stellt klar: „There was no intentional weak-entropy fallback." Aber das war auch nur noch ein schwacher Trost, und die gesamte Kommunikation wurde mit weiteren irrelevanten Rechtfertigungen über Social Media zur absoluten Katastrophe.
