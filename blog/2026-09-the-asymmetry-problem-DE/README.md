...
Eine einzige, lieblose Sicherung gab es: ein [`#error "get a HW TRNG plz"`](https://github.com/switck/libngu/blob/537519a829259622ea6b0334fbafd6cae852852f/ngu/random.c#L29), das den Build abbrechen sollte, wenn kein Hardware-Zufall da ist. Aber auch die war defekt. Sie hing an [`#ifndef MICROPY_HW_ENABLE_RNG`](https://github.com/switck/libngu/blob/537519a829259622ea6b0334fbafd6cae852852f/ngu/random.c#L28), und weil hier ein Makro im Spiel ist und keine normale Variable, prüft `#ifndef` nur, **ob** es definiert ist, nicht welchen Wert es hat. „Auf 0 definiert" zählt als „definiert", also schlug sie nie an. Und Unit-Tests, die end-to-end nachgewiesen hätten, dass der Schlüssel wirklich aus der Hardware kommt? Fehlanzeige. Der Hersteller stellt klar: „There was no intentional weak-entropy fallback." Aber das war auch nur noch ein schwacher Trost, und die gesamte Kommunikation wurde mit weiteren irrelevanten Rechtfertigungen über Social Media zur absoluten Katastrophe.
