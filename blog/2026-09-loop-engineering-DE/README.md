---
title: 'Loop Engineering: Wenn der AI-Agent sich selbst weiterschickt'
...
Schon vor dem Fortune-Auftritt kursierte eine zugespitztere Fassung. Osmani zitiert Cherny in seinem Artikel mit „I don’t prompt Claude anymore. I have loops running that prompt Claude and figuring out what to do. My job is to write loops". Ich habe diesem Satz nachgejagt: Ob Cherny ihn je genau so gesagt hat, lässt sich nicht mehr belegen. Der X-Post, auf den sich das Zitat stützt, ist gelöscht, und im Video [„Reflecting on a year of Claude Code"](https://www.youtube.com/watch?v=Hth_tLaC2j8) auf Anthropics offiziellem Kanal fällt er nicht. Was Cherny dort tatsächlich sagt (bei 10:55): „I don't talk to an agent anymore. I talk to loop or I talk to a routine and it prompts Claude for me." Die virale Fassung ist demnach nachgeschärft. Und wörtlich nehmen sollte man beides ohnehin nicht: Hier bewirbt der Chef sein eigenes Produkt. Wer selbst Schleifen laufen lässt, greift sehr wohl ein, justiert nach und bricht ab. Und die Grundlage für eine Schleife ist selbstverständlich immer noch ein Prompt. Also insgesamt doch ziemlich dick aufgetragen, wenn man mich fragt.
...
Ein paar Grenzen solltest du kennen. Die Schleife lebt in der Sitzung. Eine frische Konversation beendet sie, aber ein `--resume` holt sie zurück, solange sie nicht verfallen ist. Verfallen ist sie sieben Tage nach dem Anlegen: Dann feuert sie ein letztes Mal und löscht sich. Esc bricht einen wartenden selbstgetakteten Durchlauf ab; eine Schleife mit festem Intervall läuft dagegen weiter, bis du sie löschst. Und im selbstgetakteten Modus kann Claude von sich aus Schluss machen, wenn er die Arbeit für erledigt hält. Plant er weder einen nächsten Durchlauf noch macht er Schluss, setzt Claude Code einen einzigen Nachzügler nach etwa zwanzig Minuten und beendet die Schleife dann.

> **⚠️ Achtung:** Auf Amazon Bedrock, Claude Platform on AWS, Google Clouds Agent Platform und Microsoft Foundry brauchen die selbstgetakteten Intervalle und der eingebaute Wartungs-Prompt mindestens Claude Code v2.1.248. Auf älteren Versionen läuft ein Prompt ohne Intervall dort in einem festen Zehn-Minuten-Takt.
...
Und im Abo ist sie obendrein gratis. Anthropic verlängert den Prompt-Cache der Hauptkonversation automatisch auf eine Stunde, [solange du im Abo innerhalb deines Kontingents arbeitest](https://code.claude.com/docs/en/prompt-caching):
...
Der zwischengespeicherte Kontext bleibt damit über jede Pause hinweg warm, die eine selbstgetaktete Schleife wählen kann, denn deren Obergrenze ist genau eine Stunde. Nur ein festes Intervall darüber reißt die Lücke auf. Die Pause kostet dich nichts.
...
Eine selbstgewählte Pause von zwanzig Minuten liegt in diesem Fall weit jenseits des Fensters. Jeder Durchlauf beginnt dann damit, den kompletten Kontext neu zu verarbeiten. Genau die Pause, die dich schonen sollte, wird dadurch teuer. Und zwar deutlich: Solange der Cache warm ist, kostet der große, unveränderte Teil des Kontexts nur ein Zehntel des normalen Input-Preises. Nach einem Miss wird genau dieser Teil neu geschrieben, zum [1,25-Fachen des normalen Preises](https://platform.claude.com/docs/en/build-with-claude/prompt-caching#pricing). Verglichen mit dem günstigen Cache-Lesen ist das mehr als das Zwölffache, und das Durchlauf für Durchlauf. Wer per API oder über Bedrock arbeitet und lange Schleifen fahren will, sollte deshalb entweder feste kurze Intervalle setzen oder den einstündigen Cache selbst anfordern, etwa über die Umgebungsvariable `ENABLE_PROMPT_CACHING_1H`, die die [Cache-Doku von Claude Code](https://code.claude.com/docs/en/prompt-caching) beschreibt. Der längere Cache bezahlt Schreibzugriffe etwas teurer, erspart dafür den ständigen Neuaufbau. Oder dir sind die verbrauchten Tokens egal. Dann YOLO! 😅

## Wann sich eine Schleife lohnt

Bis hierhin ging es darum, wie Schleifen funktionieren. Die wichtigere Frage steht davor: ob du überhaupt eine brauchst. Aus dem, was oben steht, ergeben sich ein paar Bedingungen. Fehlt eine davon, kostet die Schleife mehr, als sie einbringt.

**Braucht die Aufgabe viele Anläufe?** Eine Schleife rechnet sich über viele Durchläufe. `/loop` lebt in der Sitzung und verfällt nach sieben Tagen, `/goal` endet mit der Bedingung. Für eine Sache, die mit einem Anlauf sitzt, ist ein gut gezielter Prompt schneller und billiger. Wer mit einem Durchlauf auskommt, hat kein Schleifenproblem.

**Kann etwas außer dem Agenten Nein sagen?** Das ist die härteste von allen. Der Prüfer von `/goal` ruft keine Werkzeuge auf, er sieht nur, was im Gespräch steht. Fehlt ein harter Test, Build oder Linter, bleibt als Beleg nur, was der Agent selbst über seine Arbeit berichtet. Auch der frische Prüfer von `/goal` benotet dann bloß diesen Eigenbericht. Dann läuft die Schleife womöglich munter weiter, auch wenn längst etwas kaputt ist.
...
**Trägt dein Tarif das?** Eine Schleife liest Kontext neu, probiert Dinge aus und verwirft sie. Das kostet Tokens, egal ob am Ende etwas Brauchbares herauskommt oder nicht. Wie im Abschnitt davor beschrieben, wird es außerhalb eines Abos zusätzlich teuer, weil jede längere Pause den zwischengespeicherten Kontext reißt. Loop Engineering wirkt selbstverständlich, wenn Tokens praktisch nichts kosten, und rücksichtslos, wenn jeder Durchlauf auf der Rechnung steht. Ein Hebel dagegen ist die Denk-Tiefe: Was du mit `/effort` einstellst, gilt für jeden einzelnen Durchlauf, eine Schleife multipliziert diese Wahl also. Für mechanische Aufgaben, die vor allem Ausdauer brauchen, reicht oft eine niedrigere Stufe. Das teure Denken hebst du dir für die Läufe auf, in denen wirklich etwas zu entscheiden ist.
...
So viel zu den Bedingungen. Und es gibt einen Fall, für den ich die Schleife besonders liebe. Manche Aufgaben sind schlicht hart. Sie brauchen viele Anläufe, immer wieder gegen die CI, bis endlich alles grün ist. Genau dafür lasse ich den Agenten „grinden", notfalls die ganze Nacht.
...
Dieser Fall trifft genau die Bedingungen, die eine Schleife lohnend machen: Die CI kann Nein sagen, der Agent kann seinen Code ausführen, und im Abo kosten die vielen Nachtdurchläufe nichts extra, solange du in deinem Kontingent bleibst. Kein Wunder, dass die Schleife hier glänzt.

## Ein Blick von innen

Bis hierhin stand alles in der öffentlichen Dokumentation. Jetzt geht es eine Ebene tiefer, zu den Anweisungen, die das Modell zur Laufzeit bekommt und die ich direkt aus dem Programm ausgelesen habe, ergänzt um eigene Messungen. Anthropic ändert solche Texte ohne Ankündigung, in einer späteren Version kann dort also etwas anderes stehen.
...
Beenden kann das Modell die Schleife ebenfalls selbst, mit demselben Werkzeug und dem Aufruf `stop: true`. Genau das habe ich im Test ausgelöst, sobald die Frage beantwortet war. Die Rückmeldung: `Loop stopped, cancelled 1 pending wakeup(s)`. Eine Falle steckt darin: Das beendet nur die selbstgetaktete Schleife. Eine mit festem Intervall läuft weiter und muss über `CronDelete` beendet werden.
...
So blockiert ein Command-Hook das Anhalten, solange die Tests rot sind. Der Eintrag gehört in die `settings.json`:
...
input=$(cat)
# ohne diesen Wächter blockiert der Hook jeden Anhalteversuch
printf '%s' "$input" | grep -Eq '"stop_hook_active"[[:space:]]*:[[:space:]]*true' && exit 0
if out=$(npm test 2>&1); then
  exit 0
fi
# die letzten Zeilen zurückgeben: frischer Input für den nächsten Durchlauf
printf 'Tests sind rot, mach weiter:\n%s\n' "$(printf '%s' "$out" | tail -n 20)" >&2
exit 2
```

Drei Dinge gehören dazu. Das Skript muss ausführbar sein (`chmod +x .claude/hooks/test-gate.sh`), sonst scheitert der Aufruf still und der Zug endet auch bei roten Tests. Der Wächter mit `stop_hook_active` ist keine Kür: Ohne ihn blockiert der Hook jeden Anhalteversuch, bis das Sicherheitsnetz greift und Claude Code den Zug nach acht Blockaden in Folge mit einer Warnung beendet, hochsetzbar über `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`. Und das Skript unterscheidet nicht zwischen roten Tests und einer kaputten Umgebung: Scheitert `npm test` schon an fehlenden Paketen, gilt auch das als rot.
...
Bleibt die Frage, ob das eine Eigenheit von Claude Code ist. Hier ist eine Übersicht der gängigen Werkzeuge, jeweils an ihrer eigenen Doku geprüft (Stand 3. September 2026):
...
| [OpenAI Codex](https://learn.chatgpt.com/docs/automations) | keiner | ein `/goal`-Pendant im CLI, geplante Aufgaben in der App |
...
**Codex kann es, aber woanders.** Das CLI hat laut [Befehlsreferenz](https://learn.chatgpt.com/docs/developer-commands) inzwischen Dutzende Slash-Befehle, darunter sogar ein Pendant zu `/goal` („Set, edit, pause, resume, view, or clear a task goal."). Eine Schleife ist nicht darunter. Die Zeitplanung sitzt in der App. Dort gibt es allerdings etwas, das der Schleife sehr nahe kommt, nämlich geplante Aufgaben innerhalb eines bestehenden Chats. OpenAI beschreibt sie mit einer bemerkenswerten Wortwahl: „Scheduled tasks in a chat can use minute-based intervals for active follow-up loops." Das Wort „loop" steht also auch dort.

**Im Codex-Repository liegt seit Ende Mai ein Wunsch.** [Issue #25466](https://github.com/openai/codex/issues/25466) fordert genau diese Funktion für das CLI und beschreibt sie bis in die Werkzeugnamen hinein wie bei Claude Code, inklusive `CronCreate` und `ScheduleWakeup`. Der Autor hat sie auf einem Fork bereits gebaut. Eröffnet am 31. Mai 2026 und Anfang September 2026 noch immer offen, ohne einen einzigen Kommentar.
...
- **`/loop` wartet, `/goal` nicht.** Wartest du auf etwas Fremdes wie den grünen CI-Run, der zugleich einen guten Takt vorgibt, nimm die Schleife. Arbeitest du auf einen Endzustand hin, nimm das Ziel.
...
- **Selbstständiges Abbrechen ist reines Ermessen.** Verlasse dich nicht darauf, dass der Agent bei einem Fund von allein innehält.
